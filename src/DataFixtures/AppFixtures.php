...
class AppFixtures extends Fixture
{
    public function load(ObjectManager $manager)
    {
      ...
      $patrick->setEmail("pat@free.fr");
      $patrick->setRoles(["ROLE_USER","ROLE_ADMIN"]);
      $patrick->setPassword('$2y$15$LbP4TP3ggWDGBgNzOjX8v.Mx9i1kCaEiSnJ56urHyffVFmcxSHfMy');
      $manager->persist($patrick);

      $pantxika = new User();
      $pantxika->setPrenom("Pantxika");
      $pantxika->setNom("Dagoret");
      $pantxika->setEmail("pantxika@free.fr");
      $pantxika->setRoles(["ROLE_USER"]);
      $pantxika->setPassword('$2y$15$wIR7SE2bjz44ZdtXscp7HOjJdGq7NTeDpDCqc0OyMS.PCVczerwku');
      ...
      ************************************************************/
      $intituleStage = array(
        ...
        "Stage intégrateur réseau",
        "Stage Programmation Logiciel"
      );

      /************************************************************
      ****    CREATION DES FORMATIONS ET DES STAGES ASSOCIES   ****
      ************************************************************/
      //indice pour la création d'un $tableauEntreprise
      $i=0;
      ...
      foreach ($formationStage as $codeFormation =>$titreFormation){
          // création d'une nouvelle formation
           ...
           //Création d'un tableau contenant les formations
           $tabformation[$i] = array ($formation);
           $i++;
       ...
       for ($numStage = 0; $numStage < $nbStages; $numStage++){
          ...
          $numIntituleStage = $faker->numberBetween($min = 0, $max = 5);

          $stage = new Stage();
          $stage -> setIntitule($intituleStage[$numIntituleStage]);
